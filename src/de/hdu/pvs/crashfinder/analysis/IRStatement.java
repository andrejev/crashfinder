...
/**
 * Keep the results of configuration propagation analysis. Check this thread:
...
public class IRStatement {
	...
	public static String[] ignoredPackages = new String[] { "java.", "javax.",
			"plume.", "org.apache.bcel.", "com.ibm.wala.",
			"edu.washington.cs.conf.", "gnu.trove.", "javassist.",
			"Lhj.runtime", "org.scannotation", "hj.runtime", "org.junit",
			"junit.", "gov.nasa.", };
	...
	public String getUniqueSignature() {
		return this.getMethodSig() + "#" + this.ssa.toString() + "#"
	...
	public String getDeclaringFullClassName() {
		return extractFullClassName(methodSig);
	}
	...
	public boolean shouldIgnore() {
		String fullMethodName = WALAUtils.getFullMethodName(this.s.getNode()
				.getMethod());
	...
	private int getBytecodeIndex(Statement stmt, int index) {
		try {
			return ((ShrikeBTMethod) stmt.getNode().getMethod())
					.getBytecodeIndex(index);
		} catch (Throwable e) {
	...
	@Override
	public int hashCode() {
		return this.s.hashCode() + 13 * this.ssa.hashCode() + 17
				* this.instructionIndex + 29 * this.bcIndex + 31
				* this.lineNumber;
	}
	...
	public static void dumpIRStatements(Collection<IRStatement> slice,
			PrintWriter w) {
		w.println("SLICE:\n");
		int i = 1;
		for (IRStatement irs : slice) {
			...
			String line = (i++) + "   " + s + Globals.lineSep + "\t"
